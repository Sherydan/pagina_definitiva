...
$sql ="SELECT * FROM tmp_carro WHERE cod_cliente='$cod_clientes' AND nom_cliente='$nombre'";
$cekQry = mysql_query($Sql, $coneciondb) or die("Carrito a fallado".mysql_error());
...
$txtdireccion      = str_replace("'","&acute;", $txtAlamat);
    ...
    $txtNote  =str_replace("'","&acute;", $txtNotelpon);
    ...
    if (count($pesanError)>=1) {
       ...
       $cod_cliente   =buatKode("cod","PS");
       $fecha        = date('Y-m-d');
        $mySql ="INSERT INTO reserva (no_reserva,fecha, id_cliente,nm_cliente,receptor,payment,
                 direccion,id_comuna, cuidad,pais, codigo_pos, no_telefono,email) VALUES
                ('$reserva','$fechal','$cliente','$nomcliente','$receptor','$cmbPayment','$direcciont','$cuidad','$comuna','$region','$txtpos','$txtcelular','$txtEmail')";
        $myQry= mysql_query($mySql, 'conexiondb')or die("Gagal Query1".mysql_error());
        if ($myQry) {
            //membaca data dari tmp keranjang 
            $bacaSql="SELECT *  FROM tmp_carro WHERE nm_cliente='$cliente' AND mom_cliente='$nombre'";
            $bacaQry =mysql_query($bacaSql, $koneksidb)or die("error carro".mysql_error());
            while ($bacaData =mysql_fetch_array($bacaQry)) {
                # SIMPAN DATA DARI KERANJANG BELANJA KE PEMESANAN ITEM
                $cod     = $bacaData['telefono'];
                $precio      = $bacaData['precio'];
                $numeroh    = $bacaData['numero'];

                $simpanSql ="INSERT INTO reserva_item(no_reserva, productos, precio,payment, cantidad)
                              VALUES('$no_reserva', '$productos','$precio','$cmbPayment', '$cantidad')";
                mysql_query($simpanSql, $koneksidb) or die ("error sql".mysql_error());
          
           $sqlStok="UPDATE productos SET stok=stok- $numero WHERE id_producto='$producto'";
                mysql_query($sqlStok,$koneksidb) or die("error sin stock".mysql_error());
            }

            //KOSONGKAN DATA KERANJANG MILIK PELANGGAN 
            $Sql = "DELETE FROM tmp_carro WHERE id_cliente='$cliente' AND nombren='$nombrecliente'";
            mysql_query($Sql, $conecciondb) or die("error".mysql_error());
...
$dataNama = isset($_POST['txtnombre']) ? $_POST['txtNama']: '' ;
$dataAlamat = isset($_POST['txtcorreo']) ? $_POST['txtcorreo'] :'';
$dataProvinsi =isset($_POST['comuna']) ? $_POST['comuna'] :'';
$dataKota       =isset($_POST['cuidad']) ? $_POST['ciudad']:'';
$dataKec    =isset($_POST['pais']) ? $_POST['pais'] :'';
$dataPos       =isset($_POST['pos']) ? $_POST['txtPos'] :'';
$dataNotelpon   =isset($_POST['txttelefono']) ? $_POST['txttelefono'] :'';
...
function justNumbers(e)
            ...
            <div class="span9">
                <table class="table table-striped" >
                    ...
                    <?php 
                        ...
                        $mySql ="SELECT producto.nm_producto, tmp_carro.*
                                 FROM tmp_carro LEFT JOIN producto ON tmp_carro.cod_producto=producto.cod_prodcuto
                                WHERE producto.cod_producto=tmp_carro.cod_producto AND tmp_carro.id_cliente='$codcliente'
                                ORDER BY tmp_keranjang.id";
                        $myQry = mysql_query($mySql, $conectardb)or die("error SQL".mysql_error());
                        $nomor=0;
                        while ($myData =mysql_fetch_array($myQry)) {
                            $nomor++;
                   
                            $subTotal= $myData['precio'] * $myData['catidad'];

                       
                            $totalprecio = $totalprecio + $subTotal;

                            //MENDAPATKAN TOTAL BARANG
                            $totalproducto = $totalproductg + $myData['numero'];
                    ?>
                    <tr>
                        <td align="center" width="4%"><?php echo $nomor; ?></td>
                        <td align="center"><?php echo $myData['cod_producto']; ?></td>
                    ...
                    <tr>
                        ...
                        <td align="center" bgcolor="#F5F5F5"><?php echo $totalprecio; ?></td>
                        <td align="right" bgcolor="#F5F5F5">$ <?php echo format_angka($totalproducto); ?></td>
                    </tr>
        ...
        <table class="table table-striped table-bordered">
            <tbody>
                ...
                <td width="200">
                    ...
                    <td>
                    <div class="controls">
                        <input class="span6" name="txtNama" id="inputnombre" maxlength="50" type="text" placeholder="Beneficiario" value="<?php echo  $hsl_plg['nm_pelanggan'];  ?> ">
                    </div>
                    </td>
                    </tr>
            <tr>
                ...
                <td><div class="controls">
                    <input class="span6" name="txtteefono" id="inputTelepon" type="text" maxlength="13" onkeypress="return justNumbers(event);" placeholder="Celular/Telefono" value="<?php echo $hsl_plg['no_telepon']; ?>">
                </div></td>
            </tr>
             <tr>
                <td>
                    ...
                    <td>
                    <div class="controls">
                        <input class="span6" maxlength="60" name="txtAlamat" type="text" id="inputdireccion" placeholder="Direccion" value="<?php echo $hsl_plg['alamat']; ?>">
                    </div>
               </td>
               </tr> 
            <tr>
                ...
                <td><div class="controls">
                    <input class="span3" name="txtKodePos" type="text" maxlength="10"  id="inputcodpos" onkeypress="return justNumbers(event);" placeholder="Codigo Postal" value="<?php echo $hsl_plg['kode_pos']; ?>">
                </div></td>
            </tr>
            <tr>
  ...
  <td><label class="control-label">Clave</label></td>
            ...
            <tr>
                ...
                <td><div class="controls">
                    <select type="text" id="comuna" name="kota">
                        ...
                        while ($dataKota=mysql_fetch_array($kota)) {
                            if ($dataKota['id_comuna']==$hsl_plg['id_comuna']) {
                                $cek ="selected";
                            }
                            else{
                                $cek ="";
            ...
            <tr>
                ...
                <td>
                    <div class="controls">
                        <select type="text" id="region" name="kec">
                            ...
                            while ($dataKec=mysql_fetch_array($kec)) {
                                if ($dataKec['id_pais']==$hsl_plg['id_pais']) {
                                    $cek ="selected";
                                }
                                else
                                {
        ...
        <input class="btn btn-large pull-right" id="pagar" name="btnpagar" type="submit" value="PAGAR"> 
        </form>  
            <div id="message"></div>     
           <div>
